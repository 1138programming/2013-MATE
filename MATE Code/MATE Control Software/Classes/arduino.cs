...
namespace Firmata.NET
{
    ...
    class Arduino
    {
        ...
        private const int SET_PIN_MODE = 0xF4; // set a pin to INPUT/O)
        private const int REPORT_ANALOG = 0xC0; // enable analog input by pin #UTPUT/PWM/etc
        ...
        private bool ExceptionCaugt = false;
        ...
        public Arduino(string serialPortName, Int32 baudRate, bool autoStart, int delay)
        {
            ...
            {
                MessageBox.Show("Critical Exception Has Occured: " + e.ToString());
            }
        }
        ...
        public void digitalWrite(int pin, int value)
        {
            ...
            {
                if (ExceptionCaugt == false)
                {
                    ExceptionCaugt = true;
                    MessageBox.Show("Connection with the Arduino has been lost.  Atempting to resolve.  Error: " + e.ToString());
                    AttemptResolveConnection(e);
                }
            }
            ...
            catch (Exception ge)
            {
                ...
                {
                    ExceptionCaugt = true;
                    MessageBox.Show("Critical Exception Has Occured, Unable to resolve while program is active.  All Robot Communications Stopped.  Error:" + ge.ToString());
                }
            }
        }
        ...
        public void processInput()
        {
            while (_serialPort.IsOpen)
            {
                ...
                catch (Exception ge)
                {
                    ...
                    {
                        ExceptionCaugt = true;
                        MessageBox.Show("Critical Exception Has Occured, Unable to resolve while program is active.  All Robot Communications Stopped.  :" + ge.ToString());
                    }
                }
            }
        }
        ...
        private void AttemptResolveConnection(Exception e)
        {
            try
            {
                _serialPort.Close();
                ...
                _serialPort = new SerialPort(Arduino.list().ElementAt(list().Length - 1), 57600);
                _serialPort.DataBits = 8;
                _serialPort.Parity = Parity.None;
                _serialPort.StopBits = StopBits.One;
                ...
                command = null;
                ExceptionCaugt = false;
            }
            catch (Exception re)
            {
                if (MessageBox.Show("Reconnection Attempt Failed.  Error:" + re.ToString(), "2012 MATE", MessageBoxButtons.RetryCancel) == DialogResult.Retry)
                {
                    AttemptResolveConnection(re);
                }
                else
                {
                    MessageBox.Show("Unable to resolve while program is active.  All Robot Communications Stopped. ");
                }
            }
        }
